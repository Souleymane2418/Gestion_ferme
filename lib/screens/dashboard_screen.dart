              ...
              builder: (_, snap) {
                ...
                if (data.isEmpty) {
                  return const Card(
                    child: Padding(
                      padding: EdgeInsets.all(24),
                      child: Text('Aucun animal enregistré pour le moment.'),
                    ),
                  );
                }
                return Card(
              ...
              if (sp.rappels.isEmpty) {
                return const Card(
                  child: ListTile(
                    ...
                    title: Text('Aucun rappel sanitaire à venir'),
                  ),
                );
              }
              return Card(
            ...
            Consumer<ReproductionProvider>(builder: (_, rp, __) {
              if (rp.misesBasAVenir.isEmpty) {
                return const Card(
                  child: ListTile(
                    leading: Icon(Icons.check_circle, color: Colors.green),
                    title: Text('Aucune mise bas prévue dans les 30 jours'),
                  ),
                );
              }
              return Card(
                child: Column(
                  children: rp.misesBasAVenir.take(5).map((r) {
                    return ListTile(
                      ...
                      subtitle:
                          Text('Prévu: ${dateFmt.format(r.dateAttendue!)}'),
                    );
                  }).toList(),
                ),
              );
